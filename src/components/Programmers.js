...
export default function Programmers() {
  ...
  cont [id, changeId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(item => id === item.id).name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { changeId(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
